...
public class PlayerMove : MonoBehaviour, IUnitDoAct
{
    ...
    // 자동으로 이동할 y축 속도
    [SerializeField] private float autoMoveSpeedY = 2f;  // 원하는 속도로 설정 가능
    ...
    {
        // 조이스틱의 x축 방향으로만 이동
        float moveX = movementDirection.x * movementDistance;
    ...
    private void OnMovementInput(Vector2 direction, float distance)
    {
